...
function _methodMutationAsync(methodName) {
  ...
  Mongo.Collection.prototype[methodName] = async function(...args) {
    ...
    try {
      const [validatedArgs, validationContext] =
        await getArgumentsAndValidationContext.call(this, methodName, args, true);
      try {
...
async function doValidate(collection, type, args, getAutoValues, userId, isFromTrustedCode, async) {
  ...
  if (Meteor.isClient && hasCallback) {
    callback = args[last] = await wrapCallbackForParsingServerErrors(validationContext, callback);
  }
  ...
  // TODO Maybe move this into SimpleSchema
  ...
  if (options.validate === false) {
    ...
    try {
      isValid = await validationContext.validate(docToValidate, {
