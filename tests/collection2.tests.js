...
describe('collection2', function () {
  ...
  if (Meteor.isServer) {
    ...
    it('upsert can handle query operators in the selector', async function () {
      ...
      const { numberAffected, insertedId } = await callMongoMethod(upsertQueryOperatorsTest, 'upsert', [
        {
  ...
  it('pick or omit schema fields when options are provided', async function () {
    ...
    const newId2 = await callMongoMethod(collection, 'insert', [{ bar: 'test' }, { omit: ['foo'] }])
    ...
    // Pick only 'foo'
    const result = await callMongoMethod(collection, 'update', [{ _id: newId2 },
