  ...
  describe('multiple top-level schemas', function () {
    ...
    if (Meteor.isServer) {
      ...
      it('upsert selects the correct schema', async function () {
        await callMongoMethod(products, 'insert', [{ title: 'Product one' }, { selector: { type: 'simple' } }])

        await callMongoMethod(products, 'upsert', [{ title: 'Product one', type: 'simple' },
          { $set: { description: 'This is a modified product one.' } },
          { selector: { type: 'simple' } }])

        await callMongoMethod(products, 'upsert', [{ title: 'Product two', type: 'simple' },
      ...
      it('update selects the correct schema', async function () {
        ...
        await callMongoMethod(products, 'update', [productId, {
          $set: { title: 'New product one' }
        }, { selector: { type: 'simple' } }])

        await callMongoMethod(products, 'update', [productVariantId, {
      ...
      it('allows changing schema on update operation', async function () {
        ...
        let product = await callMongoMethod(products, 'findOne', [productId])
        await callMongoMethod(products, 'update', [{ _id: product._id }, {
          $set: {
    ...
    if (Meteor.isServer) {
      ...
      it('insert selects the correct extended schema', async function () {
        ...
        const extendedProductVariant = await callMongoMethod(extendedProducts, 'findOne', [productVariantId])
      ...
      it('update selects the correct extended schema', async function () {
        ...
        await callMongoMethod(extendedProducts, 'update', [productId, {
          $set: { barcode: 'XYZ456' }
        }, { selector: { type: 'simple' } }])

        await callMongoMethod(extendedProducts, 'update', [productVariantId, {
          $set: { title: 'New productVariant one' }
        }, { selector: { type: 'simple' } }])

        const product = await callMongoMethod(extendedProducts, 'findOne', [productId])
        const productVariant = await callMongoMethod(extendedProducts, 'findOne', [productVariantId])
