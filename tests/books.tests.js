...
export default function addBooksTests () {
  ...
  if (Meteor.isServer) {
    it('bypassCollection2 5', async function () {
      ...
      await callMongoMethod(books, 'update', [id, { $set: { copies: 2 } }, { bypassCollection2: true }])
    })

    it('everything filtered out', async function () {
      try {
        await callMongoMethod(upsertTest, 'update', [{ _id: '123' }, {
          $set: {
            boo: 1
          }
        }])
      } catch (e) {
        expect(e.message).toBe('After filtering out keys not in the schema, your modifier is now empty')
      }
    })
    ...
    it('upsert works with schema that allows _id', async function () {
      ...
      await callMongoMethod(upsertTest, 'update', [{ _id: upsertTestId }, {
        $set: {
          foo: 2
        }
      }, {
