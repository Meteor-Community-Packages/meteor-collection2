...
describe('Using Zod for validation', () => {
  describe('Basic validation', () => {
    ...
    if (Meteor.isServer) {
      ...
      it('Should handle updates correctly', async () => {
        ...
        try {
          await callMongoMethod(booksCollection, 'update', [{ _id: id }, { $set: { copies: -1 } }]);
  ...
  describe('Zod property-based detection', () => {
    ...
    it('returns proper errors', async function () {
      ...
      try {
        ...
        expect(isTypeError || isRequiredError).toBe(
          true,
          'Error should mention either invalid type or required field'
        );
  ...
  describe('mixed validator handling', () => {
    if (Meteor.isServer) {
      it('formats errors with the validator attached to the schema being validated', async function () {
        const simpleCollection = new Mongo.Collection(
          `mixed_simple_${new Date().getTime()}`,
          Meteor.isClient ? { connection: null } : undefined
        );
        simpleCollection.attachSchema(
          new SimpleSchema({
            foo: { type: String }
          })
        );

        const zodCollection = new Mongo.Collection(
          `mixed_zod_${new Date().getTime()}`,
          Meteor.isClient ? { connection: null } : undefined
        );
        zodCollection.attachSchema(
          z.object({
            bar: z.string()
          })
        );
        ...
        try {
          await callMongoMethod(simpleCollection, 'insert', [{}]);
          throw new Error('Expected SimpleSchema validation error');
        } catch (error) {
          expect(error.name).toBe('ValidationError');
        ...
        try {
          await callMongoMethod(zodCollection, 'insert', [{}]);
  ...
  describe('Zod schemas with unknownKeys property', () => {
    ...
    it('should handle Zod schemas with unknownKeys property', function () {
      ...
      const productSchema = productInsertSchema.merge(hasId).merge(hasDates).merge(hasUser);
    ...
    if (Meteor.isServer) {
      it('should validate documents with branded types correctly', async function () {
        ...
        try {
          ...
          expect(false).toBe(
            true,
            'Expected validation error for invalid email but none was thrown'
          );
        } catch (error) {
          expect(error.name).toBe('ValidationError');
        ...
        try {
          ...
          expect(false).toBe(
            true,
            'Expected validation error for short username but none was thrown'
          );
        } catch (error) {
          expect(error.name).toBe('ValidationError');
    ...
    if (Meteor.isServer) {
      it('should validate documents against the extended schema', async function () {
        ...
        try {
          ...
          expect(false).toBe(
            true,
            'Expected validation error for invalid type in extension schema'
          );
        } catch (error) {
          expect(error.name).toBe('ValidationError');
          expect(error.message).toContain('assignee');
  ...
  describe('Zod modifier validation', () => {
    let modifierCollection;

    beforeEach(() => {
      modifierCollection = new Mongo.Collection(
        `zod_modifiers_${new Date().getTime()}`,
        Meteor.isClient ? { connection: null } : undefined
      );

      modifierCollection.attachSchema(
        z.object({
          title: z.string(),
          nested: z.object({
            flag: z.boolean(),
    ...
    if (Meteor.isServer) {
      it('validates dotted $set paths against the nested Zod field', async function () {
        const id = await callMongoMethod(modifierCollection, 'insert', [
          {
            title: 'Nested modifier test',
            nested: {
              flag: true,
              optionalNote: 'hello'
            },
        ...
        await callMongoMethod(modifierCollection, 'update', [
          { _id: id },
          { $set: { 'nested.flag': false } }
        ]);

        const updated = await callMongoMethod(modifierCollection, 'findOne', [{ _id: id }]);
        expect(updated.nested.flag).toBe(false);

        try {
          await callMongoMethod(modifierCollection, 'update', [
            { _id: id },
            { $set: { 'nested.flag': 'not-a-boolean' } }
          ]);
          throw new Error('Expected validation error for invalid dotted $set');
        } catch (error) {
          expect(error.name).toBe('ValidationError');
          expect(error.message).toContain('nested.flag');
          expect(error.message).toContain('boolean');
        }
      });

      it('rejects unsetting required fields and validates $inc field types', async function () {
        const id = await callMongoMethod(modifierCollection, 'insert', [
          {
            title: 'Unset modifier test',
            nested: {
              flag: true,
              optionalNote: 'hello'
            },
        ...
        try {
          ...
          throw new Error('Expected validation error for unsetting required field');
        } catch (error) {
          expect(error.name).toBe('ValidationError');
          expect(error.message).toContain('title');
        }

        await callMongoMethod(modifierCollection, 'update', [{ _id: id }, { $inc: { count: 2 } }]);
        ...
        try {
          ...
          throw new Error('Expected validation error for $inc on non-number field');
        } catch (error) {
          expect(error.name).toBe('ValidationError');
          expect(error.message).toContain('title');
